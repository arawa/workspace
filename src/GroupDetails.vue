...
<template>
	<div v-if="!$store.state.loadingUsersWaiting">
		...
		<AlertRemoveGroup v-if="showRemoveGroupModal"
			:message="t('workspace', '<b>Warning!</b> After deletion of the <b>{groupname}</b> group, its users will retain access to the <b>{spacename}</b> workspace', { groupname: getDisplaynameGroup, spacename: getSpaceName }, null, { escape: false })"
