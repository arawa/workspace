...
export default {
	...
	data() {
		return {
			mdiFolderOff,
			nextPage: true,
			numberWorkspacesPerPage: LIMIT_WORKSPACES_PER_PAGE,
		}
	},
	computed: {
		messageLoader() {
			return t('workspace', '{number} workspaces loading...', { number: this.numberWorkspacesPerPage })
		},
	},
	beforeMount() {
		this.showNextPage()
	...
	methods: {
		...
		countMaxPages() {
			return Math.ceil(this.$store.getters.countTotalWorkspaces / this.numberWorkspacesPerPage)
		},
		showNextPage() {
			const pageMax = this.countMaxPages()
			if (this.$store.getters.workspaceCurrentPage >= pageMax) {
