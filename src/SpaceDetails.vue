			...
			rename(this.$store.state.spaces[oldSpaceName], e.target[1].value)
				.then(resp => {
					...
					if (data.statuscode === 400) {
						this.$notify({
							title: t('workspace', 'Error to rename space'),
							text: t('workspace', 'Your Workspace name must not contain the following characters: [ ~ < > { } | ; . : , ! ? \' @ # $ + ( ) % \\\\ ^ = / & * ]<br/><br/>And your Workspace name must not a blank white into the begin or end its name'),
							type: 'error',
							duration: 6000,
						})
					}
