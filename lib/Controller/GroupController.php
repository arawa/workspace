...
class GroupController extends Controller {
	...
	public function removeUser($spaceId, $gid, $user) {
		...
		if ($gid === Application::GID_SPACE . Application::ESPACE_MANAGER_01 . $spaceId) {
			// Removes user from the list of users allowed to use the application when we are removing a user from a GE group
		...
		} elseif ($gid === Application::GID_SPACE . Application::ESPACE_USERS_01 . $spaceId) {
			...
			$space = $this->workspaceService->get($spaceId);
			foreach(array_keys($space['groups']) as $groupId) {
